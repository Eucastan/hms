...
func SetupTestDB(t *testing.T) *gorm.DB {
	...
	container, err := tcpostgres.Run(ctx,
		"postgres:15-alpine",
		tcpostgres.WithDatabase("patient_test_db"),
	...
	err = db.AutoMigrate(&models.Patient{}, &models.Admission{})
	require.NoError(t, err, "failed to migrate schema")

	t.Cleanup(func() {
		db.Exec("TRUNCATE TABLE patients, admissions RESTART IDENTITY CASCADE")
	})
